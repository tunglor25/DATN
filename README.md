...
cd DATN-1
```
*(Thay `<đường-dẫn-repo-github-của-bạn>` bằng URL Git thực tế của dự án).*
...
cp .env.example .env
```
*(Nếu bạn dùng Windows PowerShell hoặc CMD, bạn có thể copy bằng tay tệp tin và đổi tên lại thành `.env`)*
...
```
*Lưu ý: Lệnh này sẽ **xoá sạch** dữ liệu cũ (nếu có) và khởi tạo lại toàn bộ database từ số không cùng với Seeders.*
...
*(Ghi chú: Thay đổi email trên nếu UserSeeder của bạn tạo ra thông tin khác).*
